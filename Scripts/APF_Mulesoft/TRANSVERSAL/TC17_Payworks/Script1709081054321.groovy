...
for (def item : datosBan) {
    ...
    WebUI.callTestCase(findTestCase('APF_Mulesoft/TRANSVERSAL/TC15_Encrypt'), [('value') : item], FailureHandling.STOP_ON_FAILURE)

    //llenar lista con los datos bancarios encriptados
    datosEncrypt.add(GlobalVariable.VALUE_ENCRYP)

    //Lamada al TC Desencriptar
    WebUI.callTestCase(findTestCase('APF_Mulesoft/TRANSVERSAL/TC16_Dencrypt'), [('value') : GlobalVariable.VALUE_ENCRYP], 
